...
const CONFIG_READY = /^https:\/\/[^\s]+\.supabase\.co$/.test(SUPABASE_URL) &&
  SUPABASE_ANON_KEY.length > 20 && !SUPABASE_URL.includes('YOUR-PROJECT') && !SUPABASE_ANON_KEY.includes('YOUR_');
...
const DataService = {
  ...
  async createProfile(name, gradeRaw){
    const gradeLabel = GradeService.normalize(gradeRaw);
    const { data, error } = await supabase.from('profiles')
      .insert({ name, grade_label: gradeLabel })
      .select().single();
  ...
  async completeSession(sessionId){
    await supabase.from('sessions').update({ completed:true, state:'COMPLETED', updated_at:new Date().toISOString() }).eq('id', sessionId);
  },
  async updateSessionState(sessionId, state){
    await supabase.from('sessions').update({ state, updated_at:new Date().toISOString() }).eq('id', sessionId);
  },
  ...
  async listMessages(sessionId){
    ...
    const { data } = await supabase.from('messages').select('*').eq('session_id', sessionId).order('created_at', {ascending:false}).limit(200);
    return (data || []).reverse();
  },
  async logMistake(profileId, subjectLabel, topic, category, source){
    ...
    await supabase.from('mistake_log').insert({ profile_id: profileId, subject_label: subjectLabel, topic, category, source });
  },
  async getMistakeBreakdown(profileId){
    const { data } = await supabase.from('mistake_log').select('category, topic').eq('profile_id', profileId).order('created_at', {ascending:false}).limit(500);
    const rows = data || [];
  ...
  async seedDemoData(profileId){
    // Instant, realistic-looking data for a judge/evaluator — no need to
    // type an email, answer real questions, or wait on live Gemini calls
    // just to see what the progress/analysis screens look like populated.
    const skillRows = [
      { profile_id: profileId, subject_label: 'رياضيات', topic: 'معادلات من الدرجة الأولى', mastery: 0.82, attempts: 7, correct: 6 },
      { profile_id: profileId, subject_label: 'فيزياء', topic: 'قانون نيوتن الثاني', mastery: 0.63, attempts: 5, correct: 3 },
      { profile_id: profileId, subject_label: 'كيمياء', topic: 'التأكسد والاختزال', mastery: 0.44, attempts: 4, correct: 2 },
    ];
    const mistakeRows = [
      { profile_id: profileId, subject_label: 'فيزياء', topic: 'قانون نيوتن الثاني', category: 'UNIT_ERROR', source: 'session-turn' },
      { profile_id: profileId, subject_label: 'فيزياء', topic: 'قانون نيوتن الثاني', category: 'UNIT_ERROR', source: 'session-turn' },
      { profile_id: profileId, subject_label: 'كيمياء', topic: 'التأكسد والاختزال', category: 'CONCEPT_MISUNDERSTANDING', source: 'detect-mistake' },
      { profile_id: profileId, subject_label: 'رياضيات', topic: 'معادلات من الدرجة الأولى', category: 'SIGN_ERROR', source: 'session-turn' },
    ];
    await Promise.all([
      supabase.from('skills').insert(skillRows),
      supabase.from('mistake_log').insert(mistakeRows)
    ]);
  }
};
...
async function boot(){
  if(!CONFIG_READY){
    render(`<div class="screen center-col"><div class="warn-emoji">⚙️</div><h1 class="h">التطبيق محتاج إعداد بسيط</h1><p class="sub">كمّل إعداد SUPABASE_URL و SUPABASE_ANON_KEY في public/app.js ثم اعمل Deploy جديد.</p></div>`);
    return;
  }
  ...
  try{
    ...
    render(`<div class="screen center-col">
      ...
      <button class="btn btn-primary btn-block" onclick="location.reload()">${t('retry')}</button>
    </div>`);
  }
}

async function go(fn){ render(await fn()); }
...
async function screenProfiles(){
  ...
  <div class="screen">
    ...
    <div class="linklike" id="demoModeBtn" role="button" tabindex="0">${t('demo_mode_link')}</div>
    <div class="linklike" id="judgeModeBtn" role="button" tabindex="0">${t('judge_mode')}</div>
...
app.addEventListener('click', async (e) => {
  ...
  if(e.target.closest('#demoModeBtn')) return createDemoProfile();
  if(e.target.closest('#judgeModeBtn')) return location.href = '/competition.html';
});
async function createDemoProfile(){
  const btn = document.getElementById('demoModeBtn');
  if(btn) btn.textContent = t('demo_mode_loading');
  try{
    const p = await DataService.createProfile(t('demo_mode_name'), t('demo_mode_grade'));
    await DataService.seedDemoData(p.id);
    AIService.trackEvent('profile_created', { demo: true });
    await selectProfile(p);
  } catch(err){
    document.getElementById('profileErr').innerHTML = `<div class="banner">${escapeHtml(err.message)}</div>`;
    if(btn) btn.textContent = t('demo_mode_link');
  }
}
async function createProfile(){
  ...
  try{
    const p = await DataService.createProfile(name, grade || 'غير محدد');
...
async function submitAsk(){
  ...
  try{
    const analysis = await AIService.analyzeQuestion(q, currentProfile.grade_label);
...
async function runTurn(action, studentAnswer){
  ...
  try{
    const result = await AIService.sessionTurn({
      subject: session.subject, subjectLabel: session.subjectLabel, topic: session.topic,
      gradeLabel: session.gradeLabel, history: session.history, action, studentAnswer
    });
    document.getElementById('sys-last')?.remove();
    addAiMsg(result.aiMessage);
    await DataService.addMessage(session.id, 'ai', result.aiMessage);
